    ...
    function PatternLock(element, options) {
        ...
        svg.on('touchstart mousedown', (e) => {
            ...
            $(document).one(endEvent, (e) => {
                ...
                console.log('This is the end' + ans.length)

                if (ans.length == playword) {
                    checkcirc();
                }else{
                    console.log('salah' + passme);
                    skk = 0;
                    passme++;

                    if (passme >= 3) {
                        hint.style.display = 'block';
                        TweenMax.to(hint, 0.5, {scale: 1, ease: Back.easeOut});
                    }
...
//            e.returnValue = false;
        ...
        function discoverDot(e, target) {
            ...
            if (isAvailable(target) && !isUsed(target)) {
                ...
                switch(target.id) {
                    case 'b1':
                        cr1.style.backgroundColor = "#4caf50";
                        console.log(cr1.innerHTML);
                        putwords(cr1.innerHTML);
                        break;
                     case 'b2':
                            cr2.style.backgroundColor = "#4caf50";
                            putwords(cr2.innerHTML);
                            break;
                            case 'b3':
                                cr3.style.backgroundColor = "#4caf50";
                                putwords(cr3.innerHTML);
                                break;
                                case 'b4':
                            cr4.style.backgroundColor = "#4caf50";
                            putwords(cr4.innerHTML);
                            break;
                            case 'b5':
                            cr5.style.backgroundColor = "#4caf50";
                            putwords(cr5.innerHTML);
                            break;
                            case 'b6':
                            cr6.style.backgroundColor = "#4caf50";
                            putwords(cr6.innerHTML);
                            break;
                }
            }
        }
